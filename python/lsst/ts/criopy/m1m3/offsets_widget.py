...
class OffsetsWidget(QWidget):
    ...
    def __init__(self, m1m3: MetaSAL):
        ...
        self.dir_pad.positionChanged.connect(self._position_changed)
        ...
        for i in range(6):
            text = (
                f"Force {chr(ord('W') + i + 1)}"
                if i < 3
                else f"Moment {chr(ord('W') + i - 2)}"
            )
        ...
        row += 1
        self.offset_forces = QPushButton("Apply offset forces")
        self.offset_forces.setEnabled(False)
        self.offset_forces.clicked.connect(self._apply_offset_forces)
        data_layout.addWidget(self.offset_forces, row, 1, 1, 3)

        self.clear_offset_forces_button = QPushButton("Reset forces")
        self.clear_offset_forces_button.setEnabled(False)
        self.clear_offset_forces_button.clicked.connect(self._clear_offset_forces)
        data_layout.addWidget(self.clear_offset_forces_button, row, 4, 1, 3)

        layout.addStretch()

        self.m1m3.hardpointActuatorData.connect(self._hardpoint_actuator_data_callback)
        self.m1m3.imsData.connect(self._ims_data_callback)
        self.m1m3.preclippedOffsetForces.connect(self._preclipped_offset_forces)
        self.m1m3.appliedOffsetForces.connect(self._applied_offset_forces)
        self.m1m3.forceActuatorData.connect(self._force_actuator_callback)
        self.m1m3.detailedState.connect(self._detailed_state_callback)
    ...
    def get_force_offsets(self) -> dict[str, float]:
    ...
    @asyncSlot()
    async def _apply_offset_forces(self) -> None:
        await command(
            self,
            self.m1m3.remote.cmd_applyOffsetForcesByMirrorForce,
            **self.get_force_offsets(),
        )

    @asyncSlot()
    async def _clear_offset_forces(self) -> None:
        await command(self, self.m1m3.remote.cmd_clear_offset_forces)
        for f in self.FORCES:
            getattr(self, "forceOffsets_" + f).setValue(0)

    @asyncSlot()
    async def _position_changed(self, offsets: list[float]) -> None:
        args = {}
        for i in range(6):
    ...
    @Slot()
    def _hardpoint_actuator_data_callback(self, data: BaseMsgType) -> None:
    ...
    @Slot()
    def _ims_data_callback(self, data: BaseMsgType) -> None:
        self.__fill_row(self.ims_position, data)
        self.__ims_data = data
        self.__update_diffs()

    @Slot()
    def _preclipped_offset_forces(self, data: typing.Any) -> None:
        self.__fill_row(self.preclipped, data)

    @Slot()
    def _applied_offset_forces(self, data: typing.Any) -> None:
        self.__fill_row(self.applied, data)

    @Slot()
    def _force_actuator_callback(self, data: typing.Any) -> None:
        self.__fill_row(self.measured, data)

    @Slot()
    def _detailed_state_callback(self, data: typing.Any) -> None:
        ...
        self.offset_forces.setEnabled(enabled)
        self.clear_offset_forces_button.setEnabled(enabled)
